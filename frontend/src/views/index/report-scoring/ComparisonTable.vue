...
import { REPORT_SCORE_DIMENSIONS, type ComparisonResponse } from "@/api/report_scoring";
...
<template>
  <section class="card comparison-card">
    ...
    <div v-else class="table-wrap">
      <table>
        <thead><tr><th>智能体</th><th>已登记 / 有效评分</th><th>各报告最新总分</th><th>平均</th><th>最低</th><th>最高</th><th v-for="dimension in REPORT_SCORE_DIMENSIONS" :key="dimension.key">{{ dimension.label }}</th></tr></thead>
        <tbody><tr v-for="agent in comparison.agents" :key="agent.agent_id"><td><strong>{{ agent.display_name }}</strong><small>{{ agent.agent_id }}</small></td><td>{{ agent.registered_report_count }} / {{ agent.successfully_scored_report_count }}</td><td>{{ agent.report_scores.length ? agent.report_scores.map(item => item.total_score.toFixed(1)).join(' / ') : '—' }}</td><td class="average">{{ display(agent.average_total) }}</td><td>{{ display(agent.minimum_total) }}</td><td>{{ display(agent.maximum_total) }}</td><td v-for="dimension in REPORT_SCORE_DIMENSIONS" :key="dimension.key">{{ display(agent.dimension_averages[dimension.key]) }}</td></tr></tbody>
      </table>
    </div>
  </section>
</template>
...
table { min-width: 1700px; width: 100%; border-collapse: collapse; font-size: 12px; }
