...
把 `windows-agent-port-query.xml` 中的完整 `<ossec_config>` 块追加到文件末尾：
...
```bash
sudo cp manager-port-query-rule.xml /var/ossec/etc/rules/demo_port_query.xml
