...
PROMPT_VERSION = "report-scoring-v3.0-4"
SCORING_CONTRACT_VERSION = "report-scoring-contract-v3"

_DIMENSION_OVERVIEW = "、".join(
    f"{dimension.label}（{dimension.maximum} 分）" for dimension in SCORING_DIMENSIONS
)

SYSTEM_PROMPT = f"""你是开发期攻击溯源报告评分智能体。严格按照给定 v3.0 标准评分。
六个评分维度的正式名称为：{_DIMENSION_OVERVIEW}。
...
未发生行为核验维度的 correct_findings 和 incorrect_findings 必须引用上述原子 behavior_id，
