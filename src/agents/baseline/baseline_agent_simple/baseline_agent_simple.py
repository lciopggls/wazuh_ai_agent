...
from agents.baseline.baseline_agent_simple import nodes
from agents.baseline.baseline_agent_simple.state import BaselineSimpleState


def route_after_prepare(state: BaselineSimpleState) -> str:
    if state.get("error"):
        return "final_report"
...
def route_after_fetch(state: BaselineSimpleState) -> str:
    if state.get("archive_error") or not state.get("current_raw_logs"):
        return "final_report"
    return "analyze_batch"


def route_after_analysis(state: BaselineSimpleState) -> str:
...
def get_baseline_agent_simple(model: BaseChatModel, checkpointer=None):
    """创建仅使用固定窗口归档日志的简单攻击溯源基线。"""
    graph = StateGraph(BaselineSimpleState)
