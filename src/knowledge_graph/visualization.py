...
def get_networkx(json_dict, size_multipler=1.0):
    ...
    for obj in json_dict["triplets"]:
        if filtering(obj["Relation"], "r", True) in event_list:
            ...
            for te in techs:
                tech = obj["tactic"] + "-" + te
                tech_caption = (
                    "Others" if te == "Others" else re.search(r"T\d+", obj["technique"][0]).group()
                )
            ...
            G.add_node(object, type="entity")
            G.add_edge(
                subject, object, relation=obj["Relation"], label=obj["Relation"], dashline=True
            )

    # Guard: no non-Others events → return empty graph
    if not sentence_num_event_dict:
        return {"graph": G, "pos": {}}

    # add temporal links among events
    sorted_sentence_num_event_dict = dict(
        sorted(sentence_num_event_dict.items(), key=lambda x: x[0])
    )
    ...
    while len(queue) > 0:
        ...
        neighbour = list(G.predecessors(head_node)) + list(G.successors(head_node))
        for n in neighbour:
            if not visited[n]:
                queue.append(n)
        ...
        if nodes[head_node]["type"] == "entity":
            ...
            y = (
                max([p[1] for p in pos_nei] + [outside_length_max + vertical_margin])
                + base_vertical
            )
    ...
    sorted_TTP_labels = {
        tact: TTP_labels[tact] for tact in sorted(TTP_labels, key=lambda t: Tactic_label_order[t])
    }
    count = 0
    ...
    for p in pos2nodes:
        ...
        for n in n_l:
            pre_n = list(G.predecessors(n))
            suc_n = list(G.successors(n))
            nei_n = pre_n + suc_n
            nei_pos = 0
            nei_neg = 0
            for nn in nei_n:
...
def draw_pyvis(
    nx_graph, pos, pic_dir="./", pic_name="nx.html", xpx=800, ypx=1200, size_multipler=1.0
):
    nt = Network(f"{xpx}px", f"{ypx}px", notebook=True, directed=True)
...
def draw_one_pic(json_dir, name, save_dir="./data/pics"):
    file = f"{name}.json"
    name = file[:-5]  # remove '.json'
    with open(op.join(json_dir, file), encoding="utf-8") as f:
