...
def load_string(
    description_file_path, addtional_json_file_path=None, json_item_key=None, is_unique=False
):
    rule_str = ""
    if not op.exists(description_file_path):
        raise Exception(f"Rule path {description_file_path} doesn't exist")
    ...
    if addtional_json_file_path is not None:
        if not op.exists(addtional_json_file_path):
            raise Exception(
                f"The json path {addtional_json_file_path} of Rule path {description_file_path} doesn't exist"
            )
        with open(addtional_json_file_path) as f:
            jlist = json.load(f)
...
def load_txt(txt_file_path):
    ...
    if not op.exists(txt_file_path):
        raise Exception(f"Txt file path {txt_file_path} doesn't exist")
...
# report classifier
report_classifier_dir = os.path.join(_KG_ROOT, "template_files", "report_classifier")
...
def load_rule_type_meanings(txt_path: str, report_type_meanings_json: dict):
    ...
    rule_type_meanings = (
        ...
        + "\n".join(
            [
                f"{i+1}. {type_key}:{report_type_meanings_json[type_key]}"
                for i, type_key in enumerate(report_type_meanings_json)
            ]
        )
    )
...
def report_classifier_template(request_text, template_path=report_classifier_dir):
    if not op.exists(template_path):
        raise Exception(f'Report Classifier template directory "{template_path}" doesn\'t exist')
    # add system
    system_content = ""
    ...
    with open(op.join(template_path, "report_type_meanings.json")) as f:
        report_type_meaing_json = json.load(f)
    rules.append(
        load_rule_types(op.join(template_path, "system_rule_type.txt"), report_type_meaing_json)
    )
    rules.append(
        load_rule_type_meanings(
            op.join(template_path, "system_rule_type_meaning.txt"), report_type_meaing_json
        )
    )
    # add 'Rule n:'
...
# attack graph
attack_graph_dir = os.path.join(_KG_ROOT, "template_files", "attack_graph")
...
def save_relations():
    ...
    with open(op.join(attack_graph_dir, "relations.json")) as f:
        r_list = json.load(f)
    print(r_list)
...
def save_entities():
    ...
    with open(op.join(attack_graph_dir, "entities.json")) as f:
        r_list = json.load(f)
    print(r_list)
...
def attack_graph_template(request_text, template_path=attack_graph_dir):
    if not op.exists(template_path):
        raise Exception(f'Attack Graph template directory "{template_path}" doesn\'t exist')
    ...
    rules.append(
        load_string(
            op.join(template_path, "system_rule_entities.txt"),
            op.join(template_path, "entities.json"),
            "name",
        )
    )
    rules.append(
        load_string(
            op.join(template_path, "system_rule_relation.txt"),
            op.join(template_path, "relations.json"),
            "name",
            True,
        )
    )
    ...
    # Quantity limits for concise graphs
    rules.append(
        "For each article, extract at most 5 of the most critical security triplets. Prioritize the most significant attack behaviors and ignore redundant or trivial details."
    )
    # add 'Rule n:'
    rules = [f"Rule {i+1}: {rule}" for i, rule in enumerate(rules)]
...
# tactic
mitre_tactic_dir = os.path.join(_KG_ROOT, "template_files", "mitre_tactic")
...
def mitre_tactic_label_template(request_text, mitre, template_path=mitre_tactic_dir):
    if not op.exists(template_path):
        raise Exception(f'MITRE Tactic template directory "{template_path}" doesn\'t exist')
    # add system
    system_content = ""
...
# technique
mitre_technique_dir = os.path.join(_KG_ROOT, "template_files", "mitre_technique")
...
# technique template
def mitre_technique_label_template(
    request_text, mitre, parent_labels: dict, template_path=mitre_technique_dir, max_neg_num=5
):
    # parent_labels : {'tactic': ''}
    parent_tactic = parent_labels.get("tactic")
    if parent_tactic is None:
        raise Exception(
            "Tactic is not found in input params: parent_label, please input the parent_label as {'tactic': 'name of parent tact'}"
        )
    if parent_tactic == "Others":
        return None, None
    if not op.exists(template_path):
        raise Exception(f'MITRE Technique template directory "{template_path}" doesn\'t exist')
    ...
    if techniques is None:
        raise Exception(f"Tactic {parent_tactic} is not in MITRE")
    ...
    rules.append(load_string(op.join(template_path, "system_rule_num_output.txt")))
    rules.append(load_string(op.join(template_path, "system_rule_format.txt")))
    # Quality limits for concise labeling
    rules.append(
        "For the entire set of sentences, select at most 3 distinct MITRE techniques. Label each sentence with only the single most appropriate technique. If a sentence could match multiple techniques, choose only the best fit. Skip sentences that do not match any candidate technique."
    )
    ...
    for tech in techniques:
        ...
        if response_dict.get(tag) is None:
            ...
            response_lines.append(
                "{}:({})".format(tag, ",".join([str(i) for i in response_dict[tag]]))
            )
    # add example request
...
def technique_using_example():
    ...
    ms, of = mitre_technique_label_template(
        "your input article", mitre, parent_dict, mitre_technique_dir, 5
    )
    for m in ms:
        print(m["content"])
...
# sub technique
mitre_sub_technique_dir = os.path.join(_KG_ROOT, "template_files", "mitre_sub_technique")
...
def sub_technique_candidate_meaning_rule(sub_techniques, is_detailed_description=False):
    ...
    for i, sub_tech in enumerate(sub_techniques):
        cand_sub_tech_meanings.append(
            "{}. {}-{}:{}".format(
                i + 1,
                sub_tech["id"],
                sub_tech["name"],
                (
                    sub_tech["detailed_description"]
                    if is_detailed_description
                    else sub_tech["description"]
                ),
            )
        )
...
# technique template
def mitre_sub_technique_label_template(
    request_text, mitre, parent_labels: dict, template_path=mitre_sub_technique_dir, max_neg_num=5
):
    ...
    if parent_tactic is None:
        raise Exception(
            "Tactic is not found in input params: parent_label, please input the parent_label as {'tactic': 'name of parent tact', 'technique': 'name of parent tech'}"
        )
    if parent_technique is None:
        raise Exception(
            "Techinque is not found in input params: parent_label, please input the parent_label as {'tactic': 'name of parent tact', 'technique': 'name of parent tech'}"
        )
    if parent_tactic == "Others":
        return None
    if parent_technique == "Others":
        return None
    if not op.exists(template_path):
        raise Exception(f'MITRE Sub Technique template directory "{template_path}" doesn\'t exist')
    # add system
    system_content = ""
    system_background = load_txt(op.join(template_path, "system_role_description.txt")).strip()
    rules = []
    ...
    if sub_techniques is None:
        raise Exception(
            f"Technique {parent_technique} is not in the category of Tactic {parent_tactic}"
        )
    ...
    for sub_tech in sub_techniques:
        ...
        # select the first example as the prompt example
        sents = nltk.sent_tokenize(
            sub_tech["examples"][0]["description"]
        )  # split the example paragraph into sents
...
def sub_technique_using_example():
    ...
    ms = mitre_sub_technique_label_template(
        "your input article", mitre, parent_dict, mitre_sub_technique_dir, 5
    )
    if ms is not None:
        for m in ms:
            print(m["content"])
        ...
        print(
            "No sub-techniques found in tactic {} technique {}".format(
                parent_dict["tactic"], parent_dict["technique"]
            )
        )
    print(parent_dict)
...
stage_state_pool_dir = os.path.join(_KG_ROOT, "template_files", "stage_state_pool_summarization")


def save_state_pool():
    with open(
        op.join(stage_state_pool_dir, "stage_state_pool_description.txt"), encoding="utf-8"
    ) as f:
        text = f.read().strip()
    lines = text.split("\n")
    state_pool_des = {}
...
def state_meaning_rule(state_pool_des: dict):
    ...
    for i, state in enumerate(state_pool_des):
        des_list.append(f"{i+1}. {state}:{state_pool_des[state]}")
    rule = rule + "\n".join(des_list)
    return rule


def stage_state_pool_summarization_template(
    request_text, state_pool_des, template_path=stage_state_pool_dir
):
    if not op.exists(template_path):
        raise Exception(
            f'Stage State Pool Summarization template directory "{template_path}" doesn\'t exist'
        )
    system_content = ""
    system_background = load_txt(op.join(template_path, "system_role_description.txt")).strip()
    rules = []
...
def stage_state_pool_summarization_using_example():
    ...
    ms = stage_state_pool_summarization_template(
        "your input article", state_pool_des, stage_state_pool_dir
    )
    for m in ms:
        print(m["content"])
...
# rewrite and extract
rewrite_dir = os.path.join(_KG_ROOT, "template_files", "rewrite")
Tactic_label_order = {
    "Reconnaissance": 0,
...
def mitre_tactic_technique_description(tactics):
    ...
    for i, tact in enumerate(tactic_description):
        tactic_technique_meanings.append(
            "{}. {}:{}".format(i + 1, tact["name"], tact["description"])
        )
...
def rewriting_template(request_text, mitre, template_path=rewrite_dir):
    if not op.exists(template_path):
        raise Exception(f'Rewrite template directory "{template_path}" doesn\'t exist')
    with open(op.join(template_path, "output_format.json")) as f:
        output_format = json.load(f)
    # add system
    system_content = ""
...
# labeled text
label_entities_dir = os.path.join(_KG_ROOT, "template_files", "labeled_text")


def labeled_text_template(request_text, template_path=label_entities_dir):
    if not op.exists(template_path):
        raise Exception(
            f'Label entities in text template directory "{template_path}" doesn\'t exist'
        )
    with open(op.join(template_path, "output_format.json")) as f:
        output_format = json.load(f)
    # add system
...
def add_description_templete(numbered_triplets, numbered_texts, schema_path, parent_labels, model):
    ...
    if not op.exists(schema_path):
        raise Exception(f'Scheam directory "{schema_path}" doesn\'t exist')
    ...
    messages = [{"role": "system", "content": prompt_sys}, {"role": "user", "content": query}]
