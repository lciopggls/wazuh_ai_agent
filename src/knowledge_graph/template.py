...
def attack_graph_template(request_text, template_path=attack_graph_dir):
    ...
    rules.append("For each article, extract at most 5 of the most critical security triplets. Prioritize the most significant attack behaviors and ignore redundant or trivial details.")
    # add 'Rule n:'
    rules = ['Rule {}: {}'.format(i+1, rule) for i, rule in enumerate(rules)]
...
def mitre_technique_label_template(request_text, mitre, parent_labels: dict, template_path=mitre_technique_dir, max_neg_num=5):
    ...
    rules.append("For the entire set of sentences, select at most 3 distinct MITRE techniques. Label each sentence with only the single most appropriate technique. If a sentence could match multiple techniques, choose only the best fit. Skip sentences that do not match any candidate technique.")
