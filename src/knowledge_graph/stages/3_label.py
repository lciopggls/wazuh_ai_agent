...
from template import mitre_technique_label_template
...
def dict2line(d, joint_char=' ; '):
    return (d["Subject"] + "({})".format(d["SubjectType"]) + joint_char
            + d["Relation"] + joint_char
            + d["Object"] + "({})".format(d["ObjectType"]))


def request_technique_label(sents, mitre, parent_labels,
                            model=current_model, temperature=0, max_token=120000):
    numbered_text = '\n'.join(
        '{}: {}'.format(idx, sent['sent']) for idx, sent in enumerate(sents)
    )
    ...
    while True:
        try:
            ...
            if response.choices[0].message.tool_calls is None:
                return {
                    "triplets": [
                        {**sents[idx]['triplet'], 'technique': ["Others"],
            ...
            try:
                ...
                MAX_TECHNIQUES = 3
                all_techs = [t for techs in idx2technique_label.values()
                             for t in techs if t != "Others"]
                unique_techs = set(all_techs)
                if len(unique_techs) > MAX_TECHNIQUES:
                    tech_counts = {}
                    for t in all_techs:
                        tech_counts[t] = tech_counts.get(t, 0) + 1
                    keep_techs = set(sorted(tech_counts,
                                            key=tech_counts.get, reverse=True)[:MAX_TECHNIQUES])
                    for idx in idx2technique_label:
                        filtered = []
                        seen2 = set()
                        for t in idx2technique_label[idx]:
...
async def process_technique_files(report_dir, result_dir, mitre, request_target_files,
                                  sent_step):
    tactic_sentences = {}
    for file in request_target_files:
        ...
        for triplet in file['triplets']:
            ...
            sent_des = (dict2line(triplet) + ' '
                        + tactic_rewriting.get(tactic_label, ''))
            sent_element = {
                'sent': sent_des,
                'file_name': file_name,
                'triplet': triplet,
    ...
    for file_name in result_files:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            result = json.load(f)
        print('{}: Old count = {}, New count = {}'.format(
...
async def async_technique_requests(report_dir, result_dir, mitre, file_step,
                                   sent_step, name_filter=None):
    all_results_file = [f for f in os.listdir(report_dir)
                        if name_filter is None or f[:-5] in name_filter]
    all_results = []
    for i in range(0, len(all_results_file), file_step):
        f_files = all_results_file[i:i + file_step]
        request_target_files = []
        for file in f_files:
            with open(op.join(report_dir, file), 'r') as f:
                json_attack_graph = json.load(f)
                if (json_attack_graph.get('tactic_label') is not None
...
def main():
    ...
    asyncio.run(async_technique_requests(report_dir, result_dir, mitre, step, sent_step,
                                          name_filter))


def args_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('--report-dir', '-rpd', default=os.path.join(_KG_ROOT, 'data', '2_extract') + '/' )
    ...
    parser.add_argument('--mitre-json-path', '-mjp',
                        default=os.path.join(_KG_ROOT, 'data', 'mitre.json'))
