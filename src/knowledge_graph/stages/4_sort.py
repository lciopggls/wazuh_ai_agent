...
class TripletsGPTSorter:
    ...
    def get_triplets_with_num(self, schema_path=os.path.join(_KG_ROOT, 'sort_result_schema.json'),
                              model=current_model, use_cache=True):
        ...
        print('New count = {}, old count = {}'.format(
    ...
    def _dict2line(self, d):
        return (d["Subject"] + "({})".format(d["SubjectType"]) + " ; "
                + d["Relation"] + " ; "
    ...
    def request_triplets_order(self, tactic=None,
                               ...
                               model=current_model, temperature=0):
        if tactic is None:
            return None

        triplets_tactic = [t for t in self.result_json[0]['triplets']
                           if t['tactic'] == tactic]
        ...
        while True:
            try:
                response = client.chat.completions.create(
                    model=model, messages=ms, tools=tools,
    ...
    def get_sort_prompt(self, triplets, text,
                        schema_path=os.path.join(_KG_ROOT, 'sort_result_schema.json'),
                        model=current_model):
        ...
        query = ("The split sentences are:\n{}\n\n"
                 "The triplets are:\n{}\n\n"
                 "The mapping of triplet to the sentence it is extracted is:\n"
                 .format(numbered_text, numbered_triplets))

        messages = [
            {"role": "system",
             "content": ("You are an assistant to label which sentence each triplet "
                         "is extracted from.\n\n" + rules + "\n\n" + demonstration)},
