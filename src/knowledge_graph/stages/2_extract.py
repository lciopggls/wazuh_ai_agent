...
from template import attack_graph_template, Tactic_label_order
...
def _filter_triplets(triplets: list) -> list:
    ...
    clean = []
    for t in triplets:
        if (t.get('SubjectType', '') in EXCLUDE_TYPES
...
def request_attack_graph(bags, tactic='None', model=current_model,
                         temperature=0, max_token=120000):
    if len(bags) == 0:
                ...
                for line_text in text_content.split('\n'):
                    ...
                    if d:
                        d.update({'file_id': id_from_first_bag,
                                  'technique': ['T0000-Fake Technique']})
...
async def async_requests(report_dir, save_dir, file_idxes, all_reports_name,
                         step, current_model):
    all_results = []
...
def main():
    ...
    t = time.time()
    asyncio.run(async_requests(report_dir, save_dir, file_idxs,
                               all_reports_name, step, current_model))
    print('actual time = {:.2f}s'.format(time.time() - t))
